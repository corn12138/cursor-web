...
export const discussionLoader = (queryClient: QueryClient) => async ({ params }: LoaderFunctionArgs) => {
    const discussionId = params?.discussionId as string; // as 是类型断言
    ...
    ] as const // 两个请求同时发出 ---as const 会将数组中的每个元素都标记为只读
...
export const DiscussionRoute = () => {
    ...
    const discussion = discussionQuery.data?.data ?? null;
            ...
            >
                ...
                <div className='mt-8'>
                    <ErrorBoundary
                        fallback={
                            <div>
                                <p>Failed to load comments. Try to refresh the page.</p>
                            </div>
                        }
                    >
